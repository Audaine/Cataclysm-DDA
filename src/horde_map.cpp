...
bool horde_map::entity_at( const tripoint_om_ms &p )
{
    tripoint_om_sm submap_offset = project_to<coords::sm>( p );
    // TODO: check one then the other instead of unconditionally checking both.
    ...
    if( ( active_monster_submap == active_monster_map.end() ||
          active_monster_submap->second.empty() ) &&
        ( idle_monster_submap == idle_monster_map.end() || idle_monster_submap->second.empty() ) &&
        ( dormant_monster_submap == dormant_monster_map.end() ||
    ...
    return !active_monster_submap->second.count( entity_loc ) ||
           !idle_monster_submap->second.count( entity_loc ) ||
           !dormant_monster_submap->second.count( entity_loc );
}
...
{
    std::unordered_map <tripoint_om_sm, std::unordered_map<tripoint_abs_ms, horde_entity>> &target_map =
                mon.type->has_flag( mon_flag_DORMANT ) ? dormant_monster_map : active_monster_map;
    std::unordered_map<tripoint_abs_ms, horde_entity>::iterator result;
    point_abs_om omp;
    tripoint_om_sm sm;
    std::tie( omp, sm ) = project_remain<coords::om>( project_to<coords::sm>( p ) );
...
{
    std::unordered_map <tripoint_om_sm, std::unordered_map<tripoint_abs_ms, horde_entity>> &target_map =
                node.mapped().get_type()->has_flag( mon_flag_DORMANT ) ? dormant_monster_map : active_monster_map;
    point_abs_om omp;
    tripoint_om_sm sm;
